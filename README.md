...
| **P2 / Q2** 异构多点多架次调度 | ✅ 完成 | **16 架次（全部 C 型）/ 81.34 kWh / 完工 5.72 h / 准时 51.2%**；物理类硬约束违规 0 条，剩余 39 条全为时限类 |
| **P3 / Q3** 通信约束下运输与中继联合调度 | ✅ 完成 | 14 个中继架次 / **14-14 全程通信覆盖（100%）** / 总能耗 91.68 kWh；无通信类违规 |
| **P4 / Q4** 任务分区与资源配置 | ✅ 完成 | ★ 合法组数 K ∈ [1, 原子单元数]；本次 **2 个原子单元 → K=2 直接可行**、K=3 需拆 1 个架次 |
| **P5** 论文与 Word 生成 | ✅ 完成 | **51 页论文**（24 图 / 28 表 / **96 个 Word 原生公式对象** / 1.9 万字），`paper/` 下含 docx + PDF + 分目录图表备份 |
...
| **Q2** | 异构多点多架次调度 | **16 架次（全 C 型）/ 81.34 kWh / 完工 5.72 h / 准时率 51.2%**；★ **首批时限在本机队下物理不可行**（8 架机、14 组电池、40~50 min 充电 ⇒ 前 60 min 最多 8~10 架次，而 9 个区要求 60 min 内送达） |
| **Q3** | 通信约束下运输与中继联合调度 | ★ 实测 **14/16 架次存在直连中断**（平均中断占比 34.9%）→ **中继为必需项**；链路门限：直连 122 dB / 中继接入 116 dB / 中继回传 126 dB；运输 16 + 中继 14 架次、总能耗 91.68 kWh、联合完工 5.81 h |
| **Q4** | 任务分区与资源配置 | ★ 合法组数 **K ∈ [1, 原子单元数]**；本次 12 个多点架次形成 **2 个原子单元**（S014 独立 + 其余 14 区）⇒ **K=2 直接可行**、K=3 需拆 1 个架次。资源总规模 K=1/2/3 → **13/17/20 台·组**（分区不省资源），但单组最大工作量 10.86→7.31 h（**降低单组压力**） |
