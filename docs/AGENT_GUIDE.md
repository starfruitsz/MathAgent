...
| ★ 修正后的四问答案 | Q1 = **18 架次（可证最优）/ 75.07 kWh**；Q2 = **25 架次（A8+B10+C7，8 架全投入）/ 77.30 kWh / 完工 3.02 h / 准时 100.0%**；Q3 = **运输 25 + 中继 6 架次 / 79.65 kWh / 联合完工 3.05 h**，★ **时间轴真实覆盖 11/20 = 55.0%，中继缺口 4 架（如实上报）**；Q4 = **3 个原子单元 → K=2 与 K=3 均可直接由单元合并得到（改动 0 架次）**，资源 33/34/38 |
| ★ 校验状态 | Q1 达到下界（可证最优）；**Q2 独立校验器 18 类检查 0 违规**（含时限类！80/80 箱按时送达；装载率 79%，距理论下界 24 架次仅 1 个）；**Q3 硬违规 0 条 + 42 条软违规**（全部是"中继资源不足"，**设计如此**，属题目内在缺口，见 ADR-031） |
| ★ Q3 两个覆盖率口径（**别混用**） | `coverage_rate_geometric` = **20/20**（只说明"存在一个悬停点能覆盖"，是**上界**）｜`coverage_rate_timeline` = **11/20**（★ **唯一可上报口径**：中继在站 ∩ 所需中断区间） |
...
| ⚠️ 已推翻的历史结论 | ① 旧版曾断言「首批保障时限物理不可行」——那是 Q2 只用 2 架 C 型的产物，属建模缺陷，已修复（ADR-029）；② 旧版 Q3 曾声称「中继覆盖 100%」——那是**几何可达**口径，且校验器**从未执行**该项检查；按时间轴复核修复前只有 **0/20**，修复后 **11/20**（ADR-031）。**不要再把"100% 覆盖"写进论文** |
...
| ⚠️ 论文数字待更新 | ★ P9 改了 Q3 口径，**论文与 `otheragent/` 的 Q3 数字仍是旧口径**（20 中继架次 / 82.14 kWh / 几何 100%）。重跑命令见 `docs/PROGRESS.md`「交接备忘」 |
...
**答案（ADR-031 修复后）**：运输 **25 架次**（沿用问题二方案，其中 **20 个存在直连中断**、5 个可全程直连）、
中继 **6 架次**；运输能耗 77.30 kWh + 中继能耗 2.35 kWh = **总计 79.65 kWh**；联合完工 **3.05 h（10968.7 s）**；**平均直连中断占比 28.4%**。
实测 **20/25 需保障架次存在直连中断** ⇒ **中继是必需项**。
校验器：**硬违规 0 条**（物理/资源/通信缺陷类全绿）+ **42 条软违规**（全部为 `COMMS_RELAY_INSUFFICIENT`「中继资源不足」，**设计如此**，属题目内在缺口）。

> ## ★ 覆盖率两个口径（**别混用**，详见 **ADR-031**）
>
> | 口径 | 值 | 含义 |
> |---|---|---|
> | `coverage_rate_geometric` | **20/20 = 100%** | 「**存在**一个悬停点可覆盖该架次全部中断样本」—— 只回答**能不能连上**，是选址能力的**上界** |
> | `coverage_rate_timeline` | **11/20 = 55.0%** | ★ **唯一可上报口径**：「中继在站时段 ∩ 该架次所需中断区间」 |
>
> **结论（应写进论文）**：题目只给 **2 架**中继无人机，而按附件参数
> （可用能量 2.56 kWh ÷ 服务功率 1.10 kW ⇒ **在站上限约 140 min**）无法在时间轴上
> 保障全部 20 个架次。实测可保障 **11/20（55%）**、**中继缺口 4 架**
> （`relay_resource_shortage`）⇒ 这是**题目资源与需求的矛盾**，应作为 Q3/Q4 的
> **资源缺口**如实报告。
>
> ### ✅ 修复内容（P9，三处根因）
>
> | # | 根因 | 修法 |
> |:--:|---|---|
> | 1 | **服务区间被静默截短**：`svc_end = max(svc_start, window[1])`，中继晚到只把区间截短（截成 0 长度也不报错） | `evaluate_relay_sortie` 新增 `strict` 与 `RelayEvaluation.full_coverage/gap_s`，赶不上即返回 `None` |
> | 2 | **排班未把服务窗口纳入约束** | 新增 `plan_relay_sorties()`：按区间起点排序，选"能赶上且能耗最低"的 (点, 机, 电池)，支持 A 口径**同点共享** |
> | 3 | ★★ **校验器从未执行该项检查**（这才是缺陷被吞掉的**直接原因**）：旧 `run_q3.py` 只对"未被任何中继计划覆盖"的架次填 `outage_windows`，于是进了 `relay_plans` 的架次该字段**恒为空** ⇒ `_check_comms` 直接 `continue` | 现在**一律如实上报真实中断区间**；校验器区分软/硬违规（资源缺口 vs 排班缺陷） |
>
> **口径取 A（同点共享）的依据是题目原文**：附录 3 只写"每架**运输无人机**在任一时刻
> 只能由 G01 或**一架**中继无人机提供通信保障" —— 这是对**运输机侧**的约束，
> **未限制一架中继的服务对象数量**。
> ⚠️ 不要把它当成"自由裁量"或"放宽约束"。
>
> **另一个实测陷阱（ADR-032）**：保障需求要用**中断区间集合**，不能用**包络**
> `(首个中断样本, 末个中断样本)` —— 中间可能夹着直连可用的时段。
> 实测包络把站岗时长虚高 **39.3%**（277 vs 168 min），改用区间集合后
> 同一排班的中继能耗由 6.40 → **2.35 kWh**。
>
> 复核脚本：`python scripts\diag\q3_relay_window_audit.py`
> → `outputs/diag/q3_relay_window_audit.csv`、`outputs/q3/tables/q3_中继时间覆盖复核.csv`。

| 类别 | 位置 |
|---|---|
| **程序入口** | `src/q3_comms_relay/run_q3.py` |
| 程序模块 | `coverage.py`（轨迹逐时刻采样、悬停候选点生成、时空覆盖判定）<br>`relay.py`（中继架次几何与能耗 `evaluate_relay_sortie` / **`plan_relay_sorties` 排班** / `RelayRequest` / `PlannedRelaySortie`） |
| 原始产出 | `outputs/q3/metrics.json`、`params.json`、`feasibility.json`（**42 条软违规 / 硬 0 条**）、`run.log` |
| 原始图 | `outputs/q3/figures/q3_relay_positions.png`、`q3_gantt.png` |
| 原始表 | `outputs/q3/tables/`：`q3_直连状态诊断.csv`、`q3_中继架次.csv`、`q3_中继选址.csv`（含 `中断区间数`/`需保障时长s`）、`q3_通信保障.csv`、`q3_运输架次.csv`、**`q3_中继时间覆盖复核.csv`** |
...
资源总规模 K=1/2/3 = **33/34/38（台·组）**，组间不均衡度 0 / 1.8517 / 2.7035，
单组最大工作量 13.95 / 13.43 / **13.00 h** ⇒ **分区不省资源，但降低单组工作量峰值**。
★ **最关键的缺口在中继**：库存仅 **2 架**而 Q3 需 **6 个中继架次** ⇒ **中继缺口 4 架**（ADR-031），三种分区下都存在。
...
6. **对比与结论**：K=1/2/3 ⇒ 资源总量 33/34/38、不均衡度 0/1.8517/2.7035、单组最大工作量 13.95/13.43/13.00 h
...
| 优先级 | 事项 | 说明 |
|:--:|---|---|
| **P0** | ★★ **把 Q3 新口径写进论文与 `otheragent/`** | Q3 已改为**时间轴口径**（覆盖 **11/20 = 55%**、中继 **6** 架次、总能耗 **79.65 kWh**、联合完工 **3.05 h**、**中继缺口 4 架**），但论文/otheragent 仍是旧数字（20 中继架次 / 82.14 kWh / 几何 100%）。命令见 `docs/PROGRESS.md`「交接备忘」；跑完用 `check_paper_numbers.py` 核对 |
| ~~P0(旧)~~ | ~~修 Q3 中继「时间轴覆盖」缺陷~~ | ✅ **已完成（P9）**：三处根因全修（静默截短 / 排班不认窗口 / **校验器从未执行该项检查**），并按 A 口径重排。见 **ADR-031 / ADR-032** 与 §2.3 |
| ~~P0(旧)~~ | ~~修 Q2 的物理硬约束违规~~ | ✅ **已完成**：Q2/Q3 物理类违规 0 条；并加硬约束闸门（物理/资源/通信缺陷类一律中止产出）。见 §2.2 |
| **P1** | 改 Q2/Q3 后**必须重跑下游并同步论文** | Q3 继承 Q2、Q4 继承 Q3，链式依赖。跑完用 `python scripts\diag\check_paper_numbers.py` 核对论文数字 |
| **P1b** | （可选）改善 Q3 的中继排班 | 现为**贪心**（按区间起点排序 + 同点共享）。覆盖 11/20 的瓶颈是**2 架中继的站位时间**，而非能耗（仅用 2.35 kWh / 可用 15.36 kWh）。若要提高覆盖率，方向是**联合优化"选哪些架次保障"**（把有限站位时间投给"覆盖面最广的悬停点"），而不是继续压能耗 |
