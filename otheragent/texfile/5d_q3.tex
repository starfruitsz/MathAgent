...
\textbf{（3）选址择优。}对每个需保障架次，在能覆盖其全部中断样本且返航 SOC 达标的候选点中，选择能耗最低者。为避免在 1885 个候选点上做全轨迹扫描，算法先用一次扫描筛出直连中断样本（约占 0\%$\sim$56.2\%，5 个架次为 0），选址只在中断样本上评估；覆盖判定用稀疏探针快速否决。采样步长取 \textbf{$\Delta t=2$\,s}，在一次完整的选址扫描（1885 候选点 $\times$ 全轨迹）下本问总耗时约 \textbf{394.3\,s}（其中单架次的选址评估在毫秒量级）。
...
\textbf{20 个需保障架次}中，\textbf{全部可由单一悬停点实现全程覆盖}（无一个架次需要分段接力），因此总计产生 \textbf{20 个中继架次}，由 R01（\textbf{10 个}）与 R02（\textbf{10 个}）分别执行。通信保障方式的资源流向如图~\ref{fig:q3_sankey} 所示。
...
由图~\ref{fig:q3_sankey} 可见，25 个运输架次中 \textbf{20 个无法全程直连}，被分流至两架中继无人机承担，另 \textbf{5 个架次（20.0\%）由 G01 网关直连全程覆盖}、无需中继；R01 与 R02 的负载完全均衡（\textbf{10 : 10}），说明中继资源得到了合理分配。

\subsubsection{联合调度结果与连续性验证}

最终的联合调度方案为：运输 \textbf{25 个架次}、中继 \textbf{20 个架次}；\textbf{需保障的 20 个架次全部实现 100\% 全程连续通信覆盖}，且每个架次均由\textbf{单一悬停点}保障；运输能耗 \textbf{77.30\,kWh}、中继能耗 \textbf{4.83\,kWh}（占总量 \textbf{5.9\%}）、总能耗 \textbf{82.14\,kWh}；联合任务完成时间 \textbf{3.15\,h}（11323.3\,s）。这里运输能耗 77.30\,kWh 与问题二完全一致，体现了\textbf{后一问严格继承前一问方案}的一致性要求。联合调度时间线如图~\ref{fig:q3_joint_gantt} 所示。
...
图~\ref{fig:q3_joint_gantt} 中上段为 25 个运输架次、下段为 20 个中继架次，可见中继的服务窗口（橙色段）与运输架的飞行时段精确对齐——每个中继架次都先飞往悬停点并建链（浅色段），再在运输机需要保障的时段内悬停服务。这种“按需建链、窗口对齐”的调度方式使中继能耗仅占总量 \textbf{5.9\%}（20 个中继架次平均仅 \textbf{0.24\,kWh}），体现了联合优化的效率。
...
\textbf{校验结果。}独立校验器对本问报告 \textbf{0 条违规}——由于本问严格继承问题二的运输方案（运输架次数与能耗均与问题二相同），而问题二方案已使 80 箱全部按时送达（最大迟到量 0\,s），故本问的\textbf{时限类违规也为 0 条}；同时\textbf{通信类、能量类与资源类违规均为 0 条}，即连续通信约束得到完全满足（20 个需保障架次 20/20 全程覆盖、每个架次单点保障）。
