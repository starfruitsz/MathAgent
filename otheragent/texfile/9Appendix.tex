...
\begin{table}[htbp]
	...
	\begin{tabular}{lll}
		...
		\midrule
		问题二 & 运输架次数 & 16（全部 C 型，含 12 个多点串飞） \\
		问题二 & 总运输能耗 & 81.34\,kWh \\
		问题二 & 全部任务完成时间 & 5.73\,h（20610.4\,s） \\
		问题二 & 期望送达准时率 & 51.3\%（首批超时 13 箱） \\
		问题二 & 独立校验结论 & 物理类硬约束违规 0 条；39 条全为时限类 \\
		\midrule
		问题三 & 运输 / 中继架次数 & 16 / 14 \\
		问题三 & 需保障架次覆盖率 & 14 / 14 $=$ 100\% \\
		问题三 & 运输 / 中继 / 总能耗 & 81.34 / 10.34 / 91.68\,kWh \\
		问题三 & 联合任务完成时间 & 5.81\,h（20916.1\,s） \\
		问题三 & 平均直连中断占比 & 34.9\% \\
		\midrule
		问题四 & 原子单元（连通分量） & \textbf{2 个}（S014 独立，其余 14 区连成一片） \\
		问题四 & K$\,=\,$2 分区 & \textbf{可直接实现}（0 处改动） \\
		问题四 & K$\,=\,$3 分区 & 需拆分 1 个多点架次 \\
		问题四 & 桥接架次 & 4 个（T005、T008、T009、T014） \\
		问题四 & 资源总量（K$\,=\,$1/2/3） & 13 / 17 / 20（台$\cdot$组） \\
		问题四 & 最大组工作量（K$\,=\,$1/2/3） & 10.86 / 10.30 / 7.31\,h \\
		问题四 & 资源缺口 & K$\,=\,$1、K$\,=\,$2 均缺 1 组 C 型备用电池 \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{enumerate}[label=\arabic*.]
	...
	\item \textbf{独立复算}：问题二的能耗与 SOC 由第二套独立代码逐段重算验证，与求解器上报值完全吻合。
