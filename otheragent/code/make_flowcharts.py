...
SPECS = [
    ...
    },
    {
        ...
        "steps": [
            ...
            {"t": "box", "text": "独立复算校验\n逐段能耗、返航 SOC、资源占用区间"},
            {"t": "box", "text": "输出：16 架次（全 C 型）｜81.34 kWh\n完工 5.73 h｜准时率 51.3%（首批超时 13 箱）"},
        ],
    },
    {
        ...
        "steps": [
            ...
            {"t": "box", "text": "输出：14 个中继架次｜覆盖 14/14\n总能耗 91.68 kWh｜联合完工 5.81 h"},
        ],
    },
    {
        ...
        "steps": [
            ...
            {"t": "dec", "text": "分量数 ≥ K ？",
             "branch": {"text": "否 ⇒ 需拆分桥接架次\n每拆 1 个可多切出一组", "label": "否"}},
            {"t": "box", "text": "定位桥接架次：移除后分量数增加\n本方案共 4 个（T005/T008/T009/T014）"},
            {"t": "box", "text": "分区方案：K=2 直接可行（0 处改动）\nK=3 需拆分 1 个多点架次"},
            {"t": "box", "text": "组内资源核算：并行峰值 + 电池周转\n（资源不得跨组调配）"},
            {"t": "box", "text": "四维对比：配置规模 / 冗余 / 组间均衡 / 缺口\nK=1,2,3 ⇒ 资源总量 13 / 17 / 20"},
            {"t": "box", "text": "输出：K=2 与 K=3 两种分区方案 + 桥接架次清单\nK=1 与 K=2 均缺 1 组 C 型备用电池"},
        ],
    },
]
