...
\begin{equation}
	\label{eq:3}
	t_{gij}=\frac{h^{+}_{ij}}{v_{g}^{\uparrow}}+\frac{d_{ij}}{v_{g}^{c}}+\frac{h^{-}_{ij}}{v_{g}^{\downarrow}}
\end{equation}
...
\begin{equation}
	\label{eq:17}
	L_{path,ijt}=L_{FSPL,ijt}+L_{obs}\cdot b_{ijt},\qquad
...
\begin{equation}
	...
	\text{状态}=\begin{cases}
		...
		\text{中继}, & \text{直连不可用}\ \wedge\ A(\text{运输机}\leftrightarrow\text{中继})=1\ \wedge\ A(\text{中继}\leftrightarrow G01)=1\\
		\text{中断}, & \text{其余情况}
	\end{cases}
\end{equation}
