...
| 项 | 值 |
|---|---|
| 交付论文 | `paper/山区洪涝灾害下无人机运输与通信协同优化_论文.docx`（50 页 / 24 图 / 28 表 / 96 个可编辑公式） |
| 一键回归 | `python scripts\verify_paper.py`（构建 + 结构自检 + 页数 + 导出 PDF） |
| 测试 | `python -m pytest tests\ -v` → **227 passed** |
| 铁律 | **R1：改完即 commit，commit 即 push**（`.\scripts\commit.ps1 -Scope <...> -Type <...> -Message "..."`） |
| ★ 修正后的四问答案 | Q1 = **18 架次（可证最优）/ 75.07 kWh**；Q2 = **22 架次 / 75.06 kWh / 完工 4.04 h / 准时 62.5%**（⚠️ 校验器未通过，待修）；Q3 = **30 个中继架次 / 29-29 全程覆盖 / 99.68 kWh**；Q4 = **不存在合法 2/3 组分区** |
| ★ 最高优先级待办 | **修 Q2 的物理硬约束违规**（`outputs/q2/feasibility.json` 报 `[超出能量预算]` 等 124 条）→ 见 §2.2 与 §五 |
...
**答案（修正后）**：**22 架次**（B 型 15 + C 型 7）、总能耗 **75.06 kWh**、完工时间 **4.04 h（14537 s）**、
期望送达准时率 **62.5%**；首批违规 16 箱、期望送达违规 30 箱。
★ **独立校验器报告 `ok=false`、124 条违规** —— 其中**能量/时间/资源类违规指向数据口径问题，见下"待办"**。

> ⚠️ **接手必读**：`outputs/q2/feasibility.json` 目前是**不通过**的，且违规里含
> `[超出能量预算] 架次 T001：重算能耗 10.0383 kWh > 预算 3.2000 kWh`、
> `[返航SOC低于下限] 返航 SOC 0.0000 < 下限 0.2000` 这类**物理硬约束违规**（不属"时限类可论证"范畴）。
> 这说明 Q2 的求解器输出与校验器口径**尚未对齐**，是当前**最高优先级的待修问题**。
> 修复原则：**以校验器（独立重算）为准**，逐条定位是求解器算错还是校验器口径错，并在 `docs/MODEL_NOTES.md` 记录。

| 类别 | 位置 |
|---|---|
| **程序入口** | `src/q2_transport_schedule/run_q2.py` |
...
| 原始产出 | `outputs/q2/metrics.json`、`params.json`、**`feasibility.json`（校验报告，当前不通过）**、`run.log` |
...
实测 **29/35 架次存在直连中断**（平均中断占比 31.3%）⇒ **中继是必需项**。
...
| 优先级 | 事项 | 说明 |
|:--:|---|---|
| **P0** | **修 Q2 的物理硬约束违规** | `outputs/q2/feasibility.json` 有 `[超出能量预算]`、`[返航SOC低于下限]` 等**物理类**违规（非时限类）。须对齐求解器与校验器口径。**校验器为准**（R8） |
| **P1** | Q2 修好后重跑 **Q3 → Q4** 并同步论文 | Q3/Q4 继承 Q2 的方案，链式依赖 |
| **P2** | 论文页数 | 当前 **50 页，正好卡在 50~100 的下限**；删改内容后必须跑 `verify_paper.py` 复核 |
