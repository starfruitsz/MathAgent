...
\begin{table}[htbp]
	...
	\begin{tabular}{lll}
		...
		问题二 & 运输架次数 & 14（全部 C 型） \\
		问题二 & 总运输能耗 & 77.94\,kWh \\
		问题二 & 全部任务完成时间 & 5.18\,h（18662.2\,s） \\
		问题二 & 期望送达准时率 & 55.0\%（首批超时 17 箱） \\
		问题二 & 独立校验结论 & 物理类硬约束违规 0 条；36 条全为时限类 \\
		\midrule
		问题三 & 运输 / 中继架次数 & 14 / 14 \\
		问题三 & 需保障架次覆盖率 & 14 / 14 $=$ 100\% \\
		问题三 & 运输 / 中继 / 总能耗 & 77.94 / 9.94 / 87.88\,kWh \\
		问题三 & 联合任务完成时间 & 5.19\,h（18697.7\,s） \\
		问题三 & 平均直连中断占比 & 36.8\% \\
		\midrule
		问题四 & 合法 2 / 3 组分区 & \textbf{均可}（原子单元数 2：2 组零改动可行，3 组需拆 1 个多点架次） \\
		问题四 & 桥接架次 & 8 个（T007、T002、T003、T004、T009、T011、T013、T014） \\
		问题四 & 最小改动 & 0 个得 2 组，拆 1 个多点架次得 3 组 \\
		问题四 & 资源总量（K$\,=\,$1/2/3） & 12 / 16 / 21（台$\cdot$组） \\
		问题四 & 组间不均衡度 & 0 / 1.7736 / 1.7050 \\
		问题四 & 最大组工作量（K$\,=\,$1/2/3） & 9.94 / 9.38 / 8.93\,h \\
		问题四 & K$\,=\,$1 资源缺口 & 无（四类资源恰好满足） \\
		\bottomrule
	\end{tabular}
\end{table}
