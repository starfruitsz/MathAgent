...
| **P2 / Q2** 异构多点多架次调度 | ✅ 完成 | 35 架次 / 83.01 kWh / 完工 9.78 h / 准时 38.8%；★ **物理类硬约束违规 0 条**，剩余 49 条全为时限类（已论证为物理不可行） |
| **P3 / Q3** 通信约束下运输与中继联合调度 | ✅ 完成 | 30 个中继架次 / **29-29 全程通信覆盖** / 总能耗 99.68 kWh；剩余 49 条违规**全为时限类**（Q2 继承），无通信类违规 |
...
| **Q2** | 异构多点多架次调度 | 35 架次 / 83.01 kWh / 完工 9.78 h / 准时率 38.8%；★ **首批时限在本机队下物理不可行**（前 60 min 最多 8~10 架次，但有 9 个区要求 60 min 内送达） |
| **Q3** | 通信约束下运输与中继联合调度 | ★ 实测 **29/35 架次存在直连中断**（平均中断占比 31.0%）→ **中继为必需项**；链路门限：直连 122 dB / 中继接入 116 dB / 中继回传 126 dB；运输 35 + 中继 30 架次、总能耗 99.68 kWh、联合完工 9.90 h |
