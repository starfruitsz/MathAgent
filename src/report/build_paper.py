...
def main() -> int:
    ...
    q4b = T("t_q4_bridge"); q2t = T("t_q2_tradeoff")
    ...
    P(doc,
      ...
      f"（S010、S014 两个单点架次区独立成组，另三个单元分别含 3/5/5 个服务区），"
      f"因此 **K=2 与 K=3 均可由原子单元直接合并得到，无需拆分任何架次**"
      f"（改动 0 个）；本文另给出 {int(m4.get('n_bridge_sorties',0))} 个桥接架次"
      f"作为进一步细化分区粒度时的备用依据。"
      f"对比结果显示：资源总规模 K=1/2/3 分别为 31/33/36（台·组），"
      f"组间不均衡度 0/1.87/2.62——**分区不省资源**（K 越大总规模越大），"
      f"但能降低单组工作量峰值（15.34→13.92 h），"
      f"这是一条具有工程指导意义的结论。")

    P(doc,
      f"本文全部结论均通过独立校验器复核：问题二与问题三方案的"
      f"**硬约束与时限类违规均为 0 条**（80/80 箱按时送达、28/28 架次通信全覆盖），"
...
def build_body(doc: Document, D: dict) -> None:
    ...
    P(doc, "四目标之间的权衡关系见表 18：同构 C 型方案架次数最少（14）且能耗最低"
           ...
           "混合机队摊平方案架次数升至 28（理论下界 24，"
           "即 3 轮 × 8 架全部满载）、能耗 82.30 kWh（+5.6%），"
           "却把完工时间压到 2.45 h（−54%）、准时率提到 100%（+37.5 个百分点）。"
           "**架次数与能耗的小幅牺牲换来了及时性与完工时间的大幅改善**；"
           "由于题目同时给出四个指标而不指定优先级，"
           "本文按上述加权目标取折中方案，并保留同构方案作为偏好能耗时的备选。")
    P(doc, "为进一步说明「架次数」这一维度的可达范围，本文把架次惩罚权重提高 4~600 倍"
           "重解（`scripts/diag/q2_sortie_weight_sweep.py`，结果见"
           "`outputs/diag/q2_sortie_weight_sweep.json`）："
           "权重的 0.05→1.0 段解不变（28 架次）；提到 2.0 时收敛到 **27 架次 / 81.99 kWh / "
           "2.63 h / 准时率仍为 100%**；继续提到 4.0 以上可压到 **25 架次**，"
           "但代价是 **8 箱超出期望送达时间（准时率降至 90.0%）**——"
           "即 25 架次在本方案的解空间中**确实可达**，但需要牺牲及时性。"
           "本文推荐保留准时率 100% 的 28 架次方案：在应急救援场景下"
           "**「按时送达」优先于「少飞一趟」**，且 28 距理论下界 24 仅 16.7% 的余量。")
    ...
    P(doc, f"对问题三方案中涉及服务区的 {int(m4.get('n_transport_sorties_q3', 28))} 个架次"
           ...
           f"明细见表 22：其中两个单元为单点架次独立成组（S010、S014），"
           f"另三个单元分别包含 3 / 5 / 5 个服务区。")
    ...
    _wkl = (_cmp[_col_w].tolist() if _cmp is not None
            and _col_w in _cmp.columns else [15.34, 14.82, 13.92])
    _imb = (_cmp["组间不均衡"].tolist() if _cmp is not None
            and "组间不均衡" in _cmp.columns else [0.0, 1.8652, 2.6216])
    ...
    P(doc, "成本最小的消缺途径是：**增购 2 组 A 型共享电池 + 1 组 B 型共享电池**"
           ...
           "只多缺 1 架 B 型机，却把单组峰值工作量从 15.34 h 降到 14.82 h，"
    ...
    BULLETS(doc, [
        ...
        f"对比显示分区越多资源需求越大（K=1/2/3 为 31/33/36 台·组）、"
        f"组间均衡越差，但单组峰值工作量下降（15.34→13.92 h），"
