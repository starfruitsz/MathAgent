...
| **P3 / Q3** 通信约束下运输与中继联合调度 | ✅ 完成（**ADR-031 缺陷已修复**） | 6 个中继架次 / ★ **时间轴真实覆盖 11/20 = 55.0%** / 总能耗 79.65 kWh / 联合完工 3.05 h / **中继缺口 4 架（如实上报）**。见 **ADR-031 / ADR-032** |
| **P4 / Q4** 任务分区与资源配置 | ✅ 完成 | ★ 合法组数 K ∈ [1, 原子单元数]；本次 **3 个原子单元 → K=2 与 K=3 均可直接合并得到（改动 0 架次）**；资源 33/34/38 |
...
| **P9** ★★ Q3 中继「时间轴覆盖」缺陷修复 | ✅ 完成 | 三处根因全修（服务区间被静默截短 / 排班不认服务窗口 / **校验器从未执行该项检查**），按 **A 口径**（同点可共享）重排 ⇒ 时间轴覆盖 **0/20 → 11/20**，中继能耗 4.83 → **2.35 kWh**。见 **ADR-031 / ADR-032** |

> ## ⚠️ 论文数字待更新（P0）
> P9 改了 Q3 口径，但 **`paper/` 与 `otheragent/` 的 Q3 数字仍是旧口径**
> （20 中继架次 / 82.14 kWh / 几何覆盖 100%）。必须重跑
> `make_figures` → `make_figures2` → `build_paper` → `verify_paper.py`，
> 再用 `check_paper_numbers.py` 核对。命令见 `docs/PROGRESS.md`「交接备忘」。

> ## ★ Q3 覆盖率必须用时间轴口径（详见 `docs/DECISIONS.md` **ADR-031**）
> | 口径 | 值 | 含义 |
> |---|---|---|
> | `coverage_rate_geometric` | 20/20 = 100% | 只说明"**存在**一个悬停点能覆盖"——选址能力的**上界** |
> | `coverage_rate_timeline` | **11/20 = 55.0%** | ★ **唯一可上报口径**：中继在站 ∩ 所需中断区间 |
>
> 题目只给 **2 架**中继无人机（可用能量 2.56 kWh ÷ 服务功率 1.10 kW ⇒ **在站上限 ≈140 min**），
> 无法在时间轴上保障全部 20 个含中断的架次 ⇒ **中继缺口 4 架**，应作为**资源缺口**如实报告。
> **不得**把「几何可达」当作「时间轴已保障」上报。
> 复核脚本 `python scripts\diag\q3_relay_window_audit.py`。
...
| **Q3** | 通信约束下运输与中继联合调度 | ★ 实测 **20/25 需保障架次存在直连中断** → **中继为必需项**；链路门限：直连 122 dB / 中继接入 116 dB / 中继回传 126 dB；运输 25 + 中继 **6** 架次、总能耗 **79.65 kWh**、联合完工 **3.05 h**、平均中断占比 28.4%。★ **覆盖率口径**：几何可达 20/20（上界）≠ 时间轴真实覆盖 **11/20 = 55.0%**；题目 2 架中继（在站上限 ≈140 min）**无法**保障全部 20 个架次 ⇒ **中继缺口 4 架**，作为**资源缺口**如实报告。见 `docs/DECISIONS.md` **ADR-031 / ADR-032** |
| **Q4** | 任务分区与资源配置 | ★ 合法组数 **K ∈ [1, 原子单元数]**；本次 12 个多点架次形成 **3 个原子单元**（{S010}、{S014}、其余 13 区）⇒ **K=2 与 K=3 均可直接由单元合并得到（改动 0 架次）**。资源总规模 K=1/2/3 → **33/34/38 台·组**（分区不省资源），但单组最大工作量 13.95→13.00 h（**降低单组压力**） |
