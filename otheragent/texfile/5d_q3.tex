...
由图~\ref{fig:q3_diagnosis}(a) 可见，中断并非个别现象：\textbf{14 个运输架次全部存在直连中断}，占比 100\%（平均中断时间占比 36.8\%）；图~\ref{fig:q3_diagnosis}(c) 显示这 14 个架次\textbf{必须}引入中继，即\textbf{中继无人机是必需项而非可选项}。图~\ref{fig:q3_diagnosis}(b) 进一步表明中断与航段规模正相关，大规模架次（采样点多）的中断占比普遍更高。平均直连中断占比达 \textbf{36.8\%}，即约三分之一的飞行时间处于无直连状态。
...
由图~\ref{fig:q3_coverage}(b) 的核密度曲线可见，中断占比呈明显的右偏分布，多数架次集中在 35\%$\sim$50\% 区间，个别远距离架次超过 55\%。这一分布形态说明中断主要源于\textbf{远距离航段的中段遮挡}，而非全程不可用——中继只需覆盖飞行中间的一段即可，这为后续"单点全程覆盖"的选址策略提供了依据。
...
\textbf{（3）选址择优。}对每个需保障架次，在能覆盖其全部中断样本且返航 SOC 达标的候选点中，选择能耗最低者。为避免在 1885 个候选点上做全轨迹扫描，算法先用一次扫描筛出直连中断样本（约占 12\%$\sim$56\%），选址只在中断样本上评估；覆盖判定用稀疏探针快速否决。采样步长取 \textbf{$\Delta t=2$\,s}，在一次完整的选址扫描（1885 候选点 $\times$ 全轨迹）下耗时约 307\,s，其中单架次的选址评估在毫秒量级。
...
由图~\ref{fig:q3_siting}(a) 可见，全部悬停点\textbf{一致取离地高度 250\,m}（低于 300\,m 上限），这说明"离地越高视线越好"是选址的普遍规律——在允许范围内抬高悬停高度可以最大程度越过山脊、扩大接入段的可达范围。图~\ref{fig:q3_siting}(b) 显示单架次保障能耗集中在 0.48$\sim$1.02\,kWh，平均 0.71\,kWh。

14 个需保障架次中，\textbf{全部可由单一悬停点实现全程覆盖}，因此总计产生 \textbf{14 个中继架次}，由 R01（7 个）与 R02（7 个）分别执行。通信保障方式的资源流向如图~\ref{fig:q3_sankey} 所示。
...
由图~\ref{fig:q3_sankey} 可见，14 个运输架次\textbf{全部无法全程直连}，均需分流至两架中继无人机承担，且 R01 与 R02 的负载完全均衡（7 : 7），说明中继资源得到了合理分配。

\subsubsection{联合调度结果与连续性验证}

最终的联合调度方案为：运输 \textbf{14 个架次}、中继 \textbf{14 个架次}；\textbf{需保障的 14 个架次全部实现 100\% 全程连续通信覆盖}；运输能耗 \textbf{77.94\,kWh}、中继能耗 \textbf{9.94\,kWh}（占总量 11.3\%）、总能耗 \textbf{87.88\,kWh}；联合任务完成时间 \textbf{5.19\,h}（18697.7\,s）。这里运输能耗 77.94\,kWh 与问题二完全一致，体现了\textbf{后一问严格继承前一问方案}的一致性要求。联合调度时间线如图~\ref{fig:q3_joint_gantt} 所示。
...
图~\ref{fig:q3_joint_gantt} 中上段为 14 个运输架次、下段为 14 个中继架次，可见中继的服务窗口（橙色段）与运输架的飞行时段精确对齐——每个中继架次都先飞往悬停点并建链（浅色段），再在运输机需要保障的时段内悬停服务。这种"按需建链、窗口对齐"的调度方式使中继能耗仅占总量 11.3\%，体现了联合优化的效率。
...
\textbf{校验结果。}独立校验器对本问报告 36 条违规，\textbf{全部为时限类}（19 条超出期望送达时间、17 条首批保障箱超时），与问题二的违规集合\textbf{完全一致}——这正是本问严格继承问题二运输方案的直接体现（运输架次数与能耗均与问题二相同）；\textbf{无任何通信类、能量类或资源类违规}，即连续通信约束得到完全满足（最大迟到量 14587.6\,s，同样继承自问题二）。
