...
\begin{table}[htbp]
	...
	\begin{tabular}{lll}
		...
		问题二 & 运输架次数 & 35（全部 B 型） \\
		问题二 & 总运输能耗 & 83.01\,kWh \\
		问题二 & 全部任务完成时间 & 9.78\,h（35219.3\,s） \\
		问题二 & 期望送达准时率 & 38.8\%（首批超时 23 箱） \\
		问题二 & 独立校验结论 & 物理类硬约束违规 0 条；49 条全为时限类 \\
		\midrule
		问题三 & 运输 / 中继架次数 & 35 / 30 \\
		问题三 & 需保障架次覆盖率 & 29 / 29 $=$ 100\% \\
		问题三 & 运输 / 中继 / 总能耗 & 83.01 / 16.67 / 99.68\,kWh \\
		问题三 & 联合任务完成时间 & 9.90\,h（35651.9\,s） \\
		问题三 & 平均直连中断占比 & 31.0\% \\
		\midrule
		问题四 & 合法 2 / 3 组分区 & \textbf{不存在}（单一连通分量） \\
		问题四 & 桥接架次 & 3 个（T010、T017、T027） \\
		问题四 & 最小改动 & 拆 1 个得 2 组，拆 2 个得 3 组 \\
		问题四 & 资源总量（K$\,=\,$1/2/3） & 13 / 17 / 21（台$\cdot$组） \\
		问题四 & 组间不均衡度 & 0 / 1.7718 / 2.4707 \\
		问题四 & K$\,=\,$1 资源缺口 & 1 组 B 型备用电池 \\
		\bottomrule
	\end{tabular}
\end{table}
