...
| ★ 修正后的四问答案 | Q1 = **18 架次（可证最优）/ 75.07 kWh**；Q2 = **25 架次（A8+B10+C7，8 架全投入）/ 77.30 kWh / 完工 3.02 h / 准时 100.0%**；Q3 = **运输 25 + 中继 20 架次 / 82.14 kWh / 联合完工 3.15 h**，⚠️ **几何可达覆盖 20/20 但时间轴真实覆盖 0/20（未决缺陷 ADR-031）**；Q4 = **3 个原子单元 → K=2 与 K=3 均可直接由单元合并得到（改动 0 架次）** |
...
中继 **20 架次**；运输能耗 77.30 kWh + 中继能耗 4.83 kWh = **总计 82.14 kWh**；联合完工 **3.15 h（11323.3 s）**；**平均直连中断占比 28.4%**；中继能耗占比约 **5.9%**。
实测 **20/25 需保障架次存在直连中断**（中断占比见 `outputs/q3/metrics.json`）⇒ **中继是必需项**。
校验器 **`ok=true`、0 条违规**（通信类、能量类、时限类全部通过）；`run_q3.py` 亦已加硬约束闸门。

> ## ⚠️⚠️ 未决缺陷：覆盖率有两个口径，**不要混用**（详见 **ADR-031**）
>
> | 口径 | 值 | 含义 |
> |---|---|---|
> | `coverage_rate_geometric` | **20/20 = 100%** | 「**存在**一个悬停点可覆盖该架次全程轨迹」—— 只回答**能不能连上** |
> | `coverage_rate_timeline` | **0/20 = 0%** | 「中继在站时段 ∩ 该架次所需保障窗口」的时间重叠率 ≥ 99.9% —— 回答**那一刻在不在站** |
>
> 实测：**13 个架次的中继在运输机返航之后才到场**，平均时间重叠率仅 **14.3%**。
> 根因：① 中继排班只按最早可用资源定起点，**未把服务窗口纳入约束**（波次 1 有 8 架运输机同时起飞、中继只有 2 架，窗口必然冲突）；② `evaluate_relay_sortie` 取 `svc_start = max(link_ready, window[0])`，中继晚到只会把服务区间**截短**（截到 0 也不报错）⇒ 校验器报 0 违规，缺陷被静默吞掉。
>
> **在缺陷修好之前，论文与 README 必须同时给出两个口径并标注差异，不得把「几何可达」当作「时间轴已保障」上报。**
...
>
> **待你决定**（影响 Q3/Q4 结论，不可回避）：
> **A. 允许悬停点共享** —— 一架中继同时保障多架运输机（若几何允许），需把"共享"建模进覆盖判定；
> **B. 不允许共享** —— 2 架中继机物理上无法同时保障波次 1 的 8 个架次，覆盖率必然远低于 100%，须如实报告并给出资源缺口，Q4 的中继资源核算同步改。
...
| 优先级 | 事项 | 说明 |
|:--:|---|---|
| **P0** | ★★ **修 Q3 中继「时间轴覆盖」缺陷** | 现状：几何可达 20/20、**时间轴真实覆盖 0/20**。需先定口径（悬停点可否共享），再把服务窗口作为排班硬约束、让 `evaluate_relay_sortie` 显式失败、并给校验器加「中继在站 ∩ 运输中断」交叉检查。见 **ADR-031** 与 §2.3 |
| ~~P0(旧)~~ | ~~修 Q2 的物理硬约束违规~~ | ✅ **已完成**：Q2/Q3 物理类违规 0 条；并加硬约束闸门（16 类物理/资源/通信违规一律中止产出）。见 §2.2 |
