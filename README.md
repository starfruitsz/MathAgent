...
| **P2 / Q2** 异构多点多架次调度 | ✅ 完成 | **28 架次（A14+B8+C6，8 架全投入）/ 82.30 kWh / 完工 2.45 h / 准时 100.0%**；独立校验器 **0 违规**（18 类全通过） |
| **P3 / Q3** 通信约束下运输与中继联合调度 | ✅ 完成 | 24 个中继架次 / **24-24 全程通信覆盖（100%）** / 总能耗 86.62 kWh / 联合完工 2.84 h；无通信类违规 |
| **P4 / Q4** 任务分区与资源配置 | ✅ 完成 | ★ 合法组数 K ∈ [1, 原子单元数]；本次 **5 个原子单元 → K=2 与 K=3 均可直接合并得到（改动 0 架次）** |
| **P5** 论文与 Word 生成 | ✅ 完成 | **54 页论文**（24 图 / 29 表 / **106 个 Word 原生公式对象** / 2.1 万字），`paper/` 下含 docx + PDF + 分目录图表备份 |
...
| 文件 | 说明 |
|---|---|
| `paper/山区洪涝灾害下无人机运输与通信协同优化_论文.docx` | **正文 Word**（51 页，符合 50~100 页要求；公式为 Word 原生公式对象，**双击即可编辑**） |
...
| **Q2** | 异构多点多架次调度 | **28 架次（A14+B8+C6，8 架全投入）/ 82.30 kWh / 完工 2.45 h / 准时率 100.0%**（80/80 箱按时）；★ **首批时限可完全满足** —— 题目给的是**异构机队** A×4+B×2+C×2=8 架，与 60 min 档服务区数（8 个）相等，**先按机队槽位铺开专架次、再按最小松弛抢占首轮机位**即可全部按时。旧版"物理不可行"结论源于"只调用 2 架 C 型 + 派发键退化为能耗优先"，已修复（ADR-029） |
| **Q3** | 通信约束下运输与中继联合调度 | ★ 实测 **24/28 需保障架次存在直连中断** → **中继为必需项**；链路门限：直连 122 dB / 中继接入 116 dB / 中继回传 126 dB；运输 28 + 中继 24 架次、**24-24 全程覆盖**、总能耗 86.62 kWh、联合完工 2.84 h、平均中断占比 28.5% |
| **Q4** | 任务分区与资源配置 | ★ 合法组数 **K ∈ [1, 原子单元数]**；本次 9 个多点架次形成 **5 个原子单元**（{S010}、{S014}、{S004,S005,S008}、{S001,S002,S009,S012,S013}、{S003,S006,S007,S011,S015}）⇒ **K=2 与 K=3 均可直接由单元合并得到（改动 0 架次）**。资源总规模 K=1/2/3 → **31/33/36 台·组**（分区不省资源），但单组最大工作量 15.34→13.92 h（**降低单组压力**） |
