...
- Q3：运输 25 架次 + 中继 20 架次（20/25 = 80.0% 需中继）、覆盖 20/20 = 100%（均为单一悬停点全程覆盖，R01 : R02 = 10 : 10）、总能耗 82.14 kWh（运输 77.30 + 中继 4.83，中继占 5.9%）、联合完工 3.15 h（11323.3 s）、平均直连中断占比 28.4%；独立校验 0 违规
- ⚠️ **口径澄清**：上述 20/20 为**几何可达覆盖率**（存在悬停点可覆盖该架次全程）；按时间轴复核（中继在站时段 ∩ 所需保障窗口）**真实覆盖率为 0/20**，根因是中继排班未纳入服务窗口约束、且服务区间被静默截短。见 `docs/DECISIONS.md` ADR-031。
- Q4：3 个连通分量（原子单元：{S010}、{S014}、含 13 个服务区的主分量）⇒ K=2 与 K=3 均**零架次改动**可行；桥接架次 6 个（T019、T017、T018、T021、T024、T025）；K=1/2/3 资源总量 32/34/37，组间不均衡度 0/1.8517/2.7035，最大组工作量 13.95/13.43/13.00 h；K=1 缺口 = A、B、C 三型共享电池各 1 组，K=2 另缺 1 架 B 型运输机，K=3 再缺 1 架 B 型运输机 + 1 组 B 型电池 + 1 组中继能源组件
