...
- [x] **P2** Q2 异构无人机多点多架次运输调度 —— 35 架次 / 83.01 kWh / 完工 587 min
- [x] **P3** Q3 通信约束下的运输与中继联合调度 —— 30 中继架次 / **29-29 全程覆盖** / 99.68 kWh
- [x] **P4** Q4 救援任务分区与资源配置优化 —— **证明不存在合法 2/3 组分区** + 桥接与最小改动方案
- [x] **P5** 论文撰写与图表定稿 —— 24 图 / 28 表 / 53 页 docx + PDF + 分目录备份
- [x] **P6** 论文排版整改 —— MathType 公式 79 个 / 三线表 28 张 / 章页分页 / 图表同页
...
- [x] `src/report/equations.py` —— ★ 类 LaTeX → OMML → **MathType** 公式
- [x] `src/report/build_paper.py` —— ★ 论文装配（三线表 / 分页 / 图表同页 / 表头公式化）
- [x] 交付物：`paper/*.docx`（53 页）、`paper/*.pdf`、`论文体量报告.json`、图表清单与备份清单

### 测试
- [x] **227 passed**（物理公式逐条锁死 + 校验器负样本 + 四问端到端）
...
- [ ] 论文页数 53 页（要求 50~100）已达标；若继续补充内容需重新核算页数
