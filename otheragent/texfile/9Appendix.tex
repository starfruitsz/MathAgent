...
\begin{table}[htbp]
	...
	\begin{tabular}{lll}
		...
		\midrule
		问题三 & 运输 / 中继架次数 & 25 / 6（其中 20 个运输架次需中继） \\
		问题三 & 需保障架次覆盖率 & \textbf{时间轴口径 11 / 20 $=$ 55.0\%}（几何可达 20/20 $=$ 100\%）；\textbf{中继缺口 4 架} \\
		问题三 & 运输 / 中继 / 总能耗 & 77.30 / 2.35 / 79.65\,kWh（中继占 3.0\%） \\
		问题三 & 联合任务完成时间 & 3.05\,h（10968.7\,s） \\
		问题三 & 平均直连中断占比 & 28.4\% \\
		问题三 & 独立校验结论 & 硬违规 \textbf{0 条}；软违规 42 条（均为“中继资源不足”这一题目内在缺口） \\
		\midrule
		...
		问题四 & 资源总量（K$\,=\,$1/2/3） & 33 / 34 / 38（台$\cdot$组） \\
