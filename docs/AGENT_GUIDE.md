...
| ★ 修正后的四问答案 | Q1 = **18 架次（可证最优）/ 75.07 kWh**；Q2 = **14 架次（全 C 型）/ 77.94 kWh / 完工 5.18 h / 准时 55.0%**；Q3 = **运输 14 + 中继 14 架次 / 14-14 全程覆盖 / 87.88 kWh**；Q4 = **2 个原子单元 → K=2 直接可行，K=3 需拆 1 个架次** |
| ★ 校验状态 | Q1 达到下界（可证最优）；**Q2/Q3 物理类硬约束违规 0 条**，剩余 36 条全为时限类（已论证为资源约束下物理不可行）；Q3 通信类违规 0 条 |
...
**答案（已修正）**：**14 架次（全部 C 型）**、总能耗 **77.94 kWh**、完工时间 **5.18 h（18662 s）**、
期望送达准时率 **55.0%**；首批违规 17 箱、期望送达违规 36 箱。

★ **与问题一同源**：Q1 的最优解也是"全部 C 型、18 架次"（`L_g` 口径下 C 型远距离需减载），
两问结论互相印证。
...
> ✅ **已修复的两处历史缺陷（务必不要回退）**：
>
> **① 超载方案被静默输出**：曾出现"某个架次给 B 型机装了 235 kg（上限 30 kg）
...
>
> **② 目标函数导致机型选错（偏差更大）**：原目标用"假想同时开工"估计及时性 ⇒
> **违规箱数恒为 0**，且完工时间被估成 0.75~1.2 h（真实 5.7~9.8 h），
> 于是目标退化成"只看能耗"，把 **B 型 35 架次 / 完工 9.78 h / 准时 38.8%**
> 排在 **C 型 16 架次 / 完工 5.72 h / 准时 51.2% / 更省电** 的前面。
> 又因为调度器 `build_pools` **按机型分池**（实体机与电池不可跨机型混用），
> "混合机型"方案实际只在某一个池里调度，与单机型等价
> ⇒ 所以"三型可选"的结果曾与"仅 B 型"逐位相同。
>
> 修法（`run_q2.py::solve(fleet_mode="auto")`）：
> 分别构造并调度 **A / B / C 三个同构机队方案**，用**真实调度结果**的
> 加权多目标（违规箱数 + 迟到 + 完工时间 + 能耗 + 架次）评分，取最优。
> 诊断脚本：`scripts\diag\q2_force_type.py`（机型对比）、
> `q2_objective_vs_schedule.py`（假想目标 vs 真实调度）、
> `q2_fragmentation.py`（装箱碎片化）。
...
**答案（修正后）**：运输 **14 架次**（沿用问题二方案）、中继 **14 架次**；
**14/14 需保障架次实现全程连续通信覆盖（100%）**；
运输能耗 77.94 kWh + 中继能耗 9.94 kWh = **总计 87.88 kWh**；联合完工 **5.19 h**。
实测 **14/14 需保障架次存在直连中断**（中断占比由 `outputs/q3/metrics.json` 给出）⇒ **中继是必需项**。
校验器 `ok=false`、36 条违规 —— **全部为时限类**（均由 Q2 继承），
...
本次问题三方案的 **11 个多点架次**形成 **2 个原子单元**：
`{S014}` 与其余 14 区 ⇒ **K=2 无需改动任何架次即可行**；K=3 需拆分 **1 个**多点架次。
给出 **8 个桥接架次候选**（明细见 `paper/tables/t_q4_bridge.csv`）。
资源总规模 K=1/2/3 = **12/16/21（台·组）**，组间不均衡度 0 / 1.7736 / 1.7050，
单组最大工作量 9.94 / 9.38 / **8.93 h** ⇒ **分区不省资源，但显著降低单组工作量峰值**。
...
6. **对比与结论**：K=1/2/3 ⇒ 资源总量 12/16/21、不均衡度 0/1.7736/1.7050、单组最大工作量 9.94/9.38/8.93 h
   ⇒ **分区不省资源（总规模随 K 上升：各组要独立储备峰值），但能降低单组作业压力**。
   这对"多支救援队同时作业、各自独立补给"的现场组织方式有利，是一条**有工程指导意义的结论**。
   K=1 的缺口见 `t_q4_gap.csv`。
