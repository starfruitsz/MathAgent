...
| **P2 / Q2** 异构多点多架次调度 | ✅ 完成 | **14 架次（全部 C 型）/ 77.94 kWh / 完工 5.18 h / 准时 55.0%**；物理类硬约束违规 0 条，剩余 36 条全为时限类 |
| **P3 / Q3** 通信约束下运输与中继联合调度 | ✅ 完成 | 14 个中继架次 / **14-14 全程通信覆盖（100%）** / 总能耗 87.88 kWh；无通信类违规 |
...
| **Q2** | 异构多点多架次调度 | **14 架次（全 C 型）/ 77.94 kWh / 完工 5.18 h / 准时率 55.0%**；★ **首批时限在本机队下物理不可行**（8 架机、14 组电池、40~50 min 充电 ⇒ 前 60 min 最多 8~10 架次，而 9 个区要求 60 min 内送达） |
| **Q3** | 通信约束下运输与中继联合调度 | ★ 实测 **14/14 需保障架次存在直连中断** → **中继为必需项**；链路门限：直连 122 dB / 中继接入 116 dB / 中继回传 126 dB；运输 14 + 中继 14 架次、总能耗 87.88 kWh、联合完工 5.19 h |
| **Q4** | 任务分区与资源配置 | ★ 合法组数 **K ∈ [1, 原子单元数]**；本次 11 个多点架次形成 **2 个原子单元**（S014 独立 + 其余 14 区）⇒ **K=2 直接可行**、K=3 需拆 1 个架次。资源总规模 K=1/2/3 → **12/16/21 台·组**（分区不省资源），但单组最大工作量 9.94→8.93 h（**降低单组压力**） |
