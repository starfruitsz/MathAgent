...
| ★ 修正后的四问答案 | Q1 = **18 架次（可证最优）/ 75.07 kWh**；Q2 = **25 架次（A8+B10+C7，8 架全投入）/ 77.30 kWh / 完工 3.02 h / 准时 100.0%**；Q3 = **运输 25 + 中继 20 架次 / 20-20 全程覆盖 / 82.14 kWh / 联合完工 3.15 h**；Q4 = **3 个原子单元 → K=2 与 K=3 均可直接由单元合并得到（改动 0 架次）** |
| ★ 校验状态 | Q1 达到下界（可证最优）；**Q2/Q3 独立校验器 18 类检查全部 0 违规**（含时限类！80/80 箱按时送达；Q2 装载率 79%，距理论下界 24 架次仅 1 个架次） |
...
**答案（已修正）**：**25 架次**（A 型 8 + B 型 10 + C 型 7，**8 架无人机全部投入**）、
总能耗 **77.30 kWh**、完工时间 **3.02 h（10855.1 s）**、期望送达准时率 **100.0%**；
**首批违规 0 箱、期望送达违规 0 箱**，即 80/80 箱全部按时送达。
装载率 71%→**79%**；**理论下界 24 架次**（3 轮 × 8 架满载），本文仅高 1 个架次。
...
|---|---|---|---|---|---|---|---|
| 同构 C 型 | 15 | 81.35 | 5.51 | 47.5% | 18 | 42 | 667.5 |
| 同构 B 型 | 33 | 81.01 | 9.36 | 38.8% | 20 | 49 | 1541.8 |
| 同构 A 型 | 45 | 112.19 | 7.34 | 47.5% | 14 | 42 | 1011.8 |
| 混合-大机型优先 | 25 | 82.79 | 3.65 | 92.5% | 1 | 6 | 97.1 |
| 混合-小机型优先 | 25 | 82.70 | 3.49 | 90.0% | 0 | 8 | 103.4 |
| **混合-机队摊平（采用）** | **25** | **77.30** | **3.02** | **100.0%** | **0** | **0** | **79.3** |
...
**答案（修正后）**：运输 **25 架次**（沿用问题二方案，其中 **20 个存在直连中断**、5 个可全程直连）、
中继 **20 架次**；**20/20 需保障架次实现全程连续通信覆盖（100%）**；
运输能耗 77.30 kWh + 中继能耗 4.83 kWh = **总计 82.14 kWh**；联合完工 **3.15 h（11323.3 s）**；
**平均直连中断占比 28.4%**；中继能耗占比仅约 **5.9%**。
实测 **20/25 需保障架次存在直连中断**（中断占比见 `outputs/q3/metrics.json`）⇒ **中继是必需项**。
...
本次问题三方案的 **12 个多点架次**把 15 个服务区划分为 **3 个原子单元**：
...
资源总规模 K=1/2/3 = **32/34/37（台·组）**，组间不均衡度 0 / 1.8517 / 2.7035，
单组最大工作量 13.95 / 13.43 / **13.00 h** ⇒ **分区不省资源，但降低单组工作量峰值**。
...
6. **对比与结论**：K=1/2/3 ⇒ 资源总量 32/34/37、不均衡度 0/1.8517/2.7035、单组最大工作量 13.95/13.43/13.00 h
