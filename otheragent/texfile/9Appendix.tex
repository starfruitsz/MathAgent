...
\begin{table}[htbp]
	...
	\begin{tabular}{lll}
		...
		\midrule
		问题二 & 运输架次数 & 28（A 型 14 $+$ B 型 8 $+$ C 型 6，8 架全用） \\
		问题二 & 总运输能耗 & 82.30\,kWh \\
		问题二 & 全部任务完成时间 & 2.45\,h（8819.5\,s） \\
		问题二 & 期望送达准时率 & 100.0\%（80/80 箱全部按时送达；首批违规 0 箱、期望违规 0 箱） \\
		问题二 & 独立校验结论 & \textbf{违规 0 条}（物理类硬约束与两类时限约束全部通过） \\
		\midrule
		问题三 & 运输 / 中继架次数 & 28 / 24（其中 24 个运输架次需中继） \\
		问题三 & 需保障架次覆盖率 & 24 / 24 $=$ 100\%（均为单一悬停点全程覆盖） \\
		问题三 & 运输 / 中继 / 总能耗 & 82.30 / 4.32 / 86.62\,kWh（中继占 5.0\%） \\
		问题三 & 联合任务完成时间 & 2.84\,h（10219.3\,s） \\
		问题三 & 平均直连中断占比 & 28.5\% \\
		\midrule
		问题四 & 合法 2 / 3 组分区 & \textbf{均可零架次改动}（原子单元数 5：$K=2$ 与 $K=3$ 直接由原子单元合并得到） \\
		问题四 & 桥接架次 & 6 个（T015、T022、T003、T004、T016、T025） \\
		问题四 & 最小改动 & \textbf{0 个架次}（$K=2$ 与 $K=3$ 均为 0） \\
		问题四 & 资源总量（K$\,=\,$1/2/3） & 31 / 33 / 36（台$\cdot$组） \\
		问题四 & 组间不均衡度 & 0 / 1.8652 / 2.6216 \\
		问题四 & 最大组工作量（K$\,=\,$1/2/3） & 15.34 / 14.82 / 13.92\,h \\
		问题四 & K$\,=\,$1 资源缺口 & 1 组 A 型 $+$ 1 组 B 型共享电池（运输机、中继机与中继组件无缺口） \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{enumerate}[label=\arabic*.]
	...
	\item \textbf{独立复算}：问题二的能耗与 SOC 由第二套独立代码逐段重算验证，与求解器上报值一致（总能耗均为 82.3018\,kWh）；问题二、问题三的方案另由独立校验器逐条复核，违规 0 条。
